...
//APP -Run
khi.run(function($rootScope, $state, $urlRouter, kAuth) {

    $rootScope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams) {

        //Redirect if not logged in
        if (toState.auth && !toState.authAdmin && !toState.authMaster && !kAuth.loginCheck('Guest')) {
        ...
        } else if (toState.authAdmin && !toState.auth && !toState.authMaster && !kAuth.loginCheck('Admin')) {
        ...
        } else if (toState.authMaster && !toState.auth && !toState.authAdmin && !kAuth.loginCheck('Master')) {
    ...
    $scope.$watchGroup(['state.current.name', 'state.params', 'state.current'], function(newval, oldval) {
        ...
        if (param.type && param.id === 0) {
            ...
            if (name === 'Guest.Dashboard') {
                ...
                $scope.title = 'New Schedule / Life And Ministry';

            } else if (name === 'Admin.UpdateLM') {

                $scope.title = 'Update Schedule / Life And Ministry';
